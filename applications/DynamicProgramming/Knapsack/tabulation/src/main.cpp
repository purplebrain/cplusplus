  ...
  for (int idxWt = 0; idxWt < (gInputSize+1); idxWt++) {
    for (int cLeft = 0; cLeft < (gInputCapacity+1); cLeft++) {
      if ((idxWt == 0) || (cLeft == 0)) {
        ...
        // GENERIC CONDITION (based on Recursive Formula)
        cout << "Generic Condition #2 (idx, cLeft) = (" << idxWt << "," << cLeft << ")" <<endl;
...
main (int argc, char *argv[])
{
  while(1) {
    ...
    if (VAL) {
      cout << "For Knapsack Capacity " << gInputCapacity << " we can fill value up to " << VAL << endl;
    } else {
